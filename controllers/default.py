...
def curso():
    ...
    cursos = db((db.curso.instrutor == db.auth_user.id) & (db.curso.ativo == True) & ((db.curso.vaga - db(db.ownership.curso == db.curso.id).count()) > 0))
...
@auth.requires(auth.has_membership('aluno'))
def participa():
    db.ownership.insert(curso=request.args(0), users=request.args(1))
